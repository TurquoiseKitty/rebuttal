...
def get_scaled_data(arg_data, arg_seed, recal_prop):
    dataset_base_path = "datasets/real_data/"
    dataset_name = arg_data
    ...
    if arg_data in log_transform_datasets or need_scaling:
        y = np.log(y - min(y) + 1)
    ...
    return scale_data_wrapper(x,y,arg_seed, recal_prop)
...
def scale_data_wrapper(x,y, arg_seed, recal_prop):
    test_ratio = 0.1
    return scale_data(x, y, arg_seed, recal_prop, test_ratio)


def scale_data(x,y, seed, recal_prop, test_size=0.1):
    x_train, x_te, y_train, y_te = train_test_split(
        x, y, test_size=test_size, random_state=seed)
    x_tr, x_va, y_tr, y_va = train_test_split(
        x_train, y_train, test_size=recal_prop, random_state=seed)
