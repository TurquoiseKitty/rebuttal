...
def interval_MACE(
    y_out,
    Y,
    q_interval = np.array([0.05, 0.95]), 
):
    

    assert y_out.shape == (len(q_interval), len(Y))

    exp_quants_1, obs_quants_1 = obs_vs_exp(Y, np.array([q_interval[0]]), y_out[:1])
    exp_quants_2, obs_quants_2 = obs_vs_exp(Y, np.array([q_interval[1]]), y_out[1:])

    exp_quants = exp_quants_2 - exp_quants_1
    obs_quants = obs_quants_2 - obs_quants_1
...
def interval_AGCE(
    ...
    for _ in range(num_trials):
        ...
        for g_idx in range(num_group_draws):
            ...
            MACE_error = interval_MACE(y_out[:,rand_idx], Y[rand_idx], q_interval= q_interval).item()
